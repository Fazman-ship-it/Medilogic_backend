...
class ConnectionManager:
    def __init__(self):
        ...
        self.active_admins: Dict[int, List[WebSocket]] = {}  # org_id → list of admin sockets
        self.active_drivers: Dict[int, WebSocket] = {}       # user_id → driver socket

    async def connect_admin(self, websocket: WebSocket, organization_id: int):
    ...
    def disconnect_admin(self, websocket: WebSocket, organization_id: int):
    ...
    async def broadcast_to_admins(self, organization_id: int, message: dict):
        if organization_id in self.active_admins:
