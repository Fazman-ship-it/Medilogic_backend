...
async def get_driver_trip_confirmation(
    ...
    if trip.is_delivered or str(trip.status).lower() == "delivered":
        return {
            ...
            "trip_label": f"{trip.client_name} — {(
                ...
                else (str(trip.delivery_type) if trip.delivery_type else 'Unspecified')
            )}",
            "delivery_type": (
                trip.custom_delivery_description
                if trip.delivery_type and str(trip.delivery_type).lower() == "others"
                else (str(trip.delivery_type) if trip.delivery_type else None)
            ),
    ...
    # ✅ IMPORTANT: Do NOT keep re-issuing token every time unless truly expired
    if (not confirmation.access_token) or (confirmation.token_expires_at and confirmation.token_expires_at < now_utc()):
        from uuid import uuid4
    ...
    # ✅ Use the stable access_token, NOT a new JWT each request
